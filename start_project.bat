...
call "%~dp0src\backend\.venv\Scripts\python.exe" -m app.seeds.seed_demo_data --db-url sqlite:///./cadastral_dev.db

echo.
echo [*] Launching Backend API Server on http://127.0.0.1:8000 ...
start "3D Cadastre Backend API" cmd /k "cd /downloads "%~dp0src\backend" && .venv\Scripts\activate && uvicorn app.main:app --host 127.0.0.1 --port 8000 --reload"

echo.
echo [*] Launching Frontend Web Portal on http://127.0.0.1:5173 ...
start "3D Cadastre Frontend Portal" cmd /k "cd /downloads "%~dp0src\frontend" && npm run dev -- --host 127.0.0.1 --port 5173"
